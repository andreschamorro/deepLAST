...
def _create_check_dir(options) -> str:
    ...
    checkpoint_dir = os.path.join(options.checkpoint_dir, "dnavec_training-{}".format(now))
...
def build_model(options: Options, logger, prev_checkpoint=None, continue_train=True):
    ...
    if checkpoints:
        ...
        logger.info('Dnavec restoring from {} epoch'.format(latest_checkpoint))
        return Word2Vec.load(latest_checkpoint), True
    else:
        return Word2Vec(vector_size=options.vector_size, workers=options.workers), False
...
def run(prev_checkpoint=None, continue_train=True, save_vocab=False, save_model=True):
    dnavec_options = resource_filename(
            'configs',
            'dnavec.json'
    )
    options = Options.get_options_from_json(dnavec_options)

    now = datetime.utcnow().strftime("%Y%m%d%H%M%S")
    log_dir = os.path.join(options.logs_dir, 'logs-{}'.format(now))
    logger = Logger.get_logger('dnavec', log_dir)
    ...
    if save_model:
        ...
        write_vec(model, os.path.join(checkpoint_dir, 'dnavec_model'))
