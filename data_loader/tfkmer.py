...
class KmerTokenizer(object):
    ...
    """
    def __init__(self, k, max_vocab_size=0, min_count=10, sample=1e-3):
        """Constructor.
    ...
    def _build_raw_vocab(self, filenames):
        ...
        for fn in filenames:
            with bgzf.open(fn, 'r') as fa:
                for f in SeqIO.parse(fa, "fasta"):
                    lines.append(str(f.seq))

        raw_vocab = collections.Counter()
        for line in lines:
            raw_vocab.update(self._kmer(self._standardization(line)))
    ...
    def encode(self, string):
        ...
        """
        tokens = self._kmer(self._standardization(string))
        ids = [self._vocab[token] if token in self._vocab else OOV_ID 
                for token in tokens]
        return ids


class Kmer2VecDatasetBuilder(object):
...
def generate_instances(
    ...
    #result_array = tf.nest.map_structure(
    #        lambda ta: tf.stop_gradient(ta), 
    #        tf.while_loop(lambda i, ta: i < size,
    #            per_target_fn,
    #            [0, init_array]))
